...
class LoginView(APIView):
    ...
    Rate limiting: 3 failed attempts per email / 15 minutes.
    """
    ...
    def post(self, request: Request) -> Response:
        ...
        if not result.success:
            error = result.error or "Ошибка входа"
            if "слишком много" in error.lower():
                return Response(
                    {"detail": error},
                    status=status.HTTP_429_TOO_MANY_REQUESTS,
                )
            if "заблокирован" in error.lower():
                return Response(
                    {"detail": error},
...
class RefreshView(APIView):
    ...
    Accepts a valid refresh token and issues a new access+refresh pair.
    ...
    def post(self, request: Request) -> Response:
        ...
        service = AuthenticationService()
        result = service.refresh_token(serializer.validated_data["refresh_token"])

        if not result.success:
            error = result.error or "Ошибка обновления токена"
            # Revoked or expired tokens → 401
            if "отозван" in error or "устарел" in error:
                return Response(
                    {"detail": error},
                    status=status.HTTP_401_UNAUTHORIZED,
                )
...
class LogoutView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        Returns:
            200: Logout successful
            400: Missing token
            401: Not authenticated / token not found
        """
        # Explicitly require Bearer auth
        auth_header = request.headers.get("Authorization", "")
        if not auth_header.startswith("Bearer "):
            return Response(
                {"detail": "Требуется авторизация"},
        ...
        token_service = TokenService()
        try:
            token_service.decode_access_token(access_token)
        except Exception:  # noqa: BLE001
            return Response(
                {"detail": "Неверный токен"},
                status=status.HTTP_401_UNAUTHORIZED,
...
class MeView(APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        Args:
            request: DRF request with Authorization: Bearer <token>
